"""Rich rendering helpers for token usage statistics."""
...
def render_daily_usage_statistics(report: DailyUsageStatistics, console: Console) -> None:
    ...
    sorted_keys = sorted(report.usage_by_model_day.keys(), key=lambda item: (item[1], item[0]))
    ...
    overall_data = sorted(report.overall_usage.items())
...
def _print_usage_table(
    ...
    for key, stats in data:
        total_stats += stats
        total_tokens = stats.input_tokens + stats.cached_tokens + stats.output_tokens + stats.thoughts_tokens
        ...
        if show_date:
            ...
            row_args.extend([date_str, model_name])
        else:
            row_args.append(str(key))
    ...
    table.columns[
        7 + col_offset
    ].footer = f"{(total_stats.input_tokens + total_stats.cached_tokens + total_stats.output_tokens + total_stats.thoughts_tokens):,}"
